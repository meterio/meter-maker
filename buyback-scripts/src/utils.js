...
const transferMTRToMeterMaker = async()=>{
  ...
  const receipt = await mtrSysContract.methods.transfer(metermakerAddr, mtrAmount.toFixed(0)).send({from:ownerAddr})
  return receipt
}
...
const callBuybackOnMeterMaker = async()=>{
  ...
  const receipt = await metermakerInst.methods.buybackMTRG().send({from:ownerAddr})
  return receipt
}
